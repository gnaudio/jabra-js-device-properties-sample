...
const speechAnalyticsState = {
    customerSpeaking: undefined,
    agentSpeaking: undefined,
    microphoneMuteState: undefined
};
 ...
 */
async function readProperties(device) {
    try {
 ...
 */
function watchProperty(device, property, handleValue) {
 ...
 */
async function updateProperty(device, propertyName, value) {
    try {
...
async function observeAudioTelemetry(device) {
    ...
    function updateSpeechAnalyticsState(key, value) {
 ...
 */
async function observeBatteryCallStateTelemetry(device) {
    // List the Jabra device properties and prepare them for use on the device
    const propertyNames = [
        "batteryLevel",
        "isCharging",
 ...
 */
async function setupSettingsInputFields(device) {
    try {
        const propertyNames = [
            "sidetoneEnabled"
        ];
